    ...
    CurrentMovementState(STOP)
{
    ...
    angle_threshold = 37.5;
    GoStraightAngleRangeInDegrees.Set(-angle_threshold, angle_threshold);
}
...
void iAntBaseController::SetNextMovement() {
  ...
  cout << "==========" << endl;
	...
	argos::CRadians TargetHeadingTolerance(0.1);

	cout << "Target position: <" << TargetPosition.GetX() << ", " << TargetPosition.GetY() << ">" << endl;
	cout << "Current position: <" << GetPosition().GetX() << ", " << GetPosition().GetY() << ">" << endl;
	...
	cout << "TargetHeadingTolerance: " << TargetHeadingTolerance << endl;
	
        if(distanceToTarget > TargetDistanceTolerance) {
	  if(headingToTargetError < -TargetHeadingTolerance) 
	    {
	    cout << "Turning left: angle " <<  -ToDegrees(headingToTargetError).GetValue() << endl;
	    PushMovement(LEFT, -ToDegrees(headingToTargetError).GetValue());
	  } 
	  else if(headingToTargetError > TargetHeadingTolerance)
	    {
	      cout << "Turning right: angle " <<  ToDegrees(headingToTargetError).GetValue() << endl;
	      PushMovement(RIGHT, ToDegrees(headingToTargetError).GetValue());
            }
	  else
	    {
	      cout << "Going Forward: distance to target " << distanceToTarget << endl;
	      PushMovement(FORWARD, distanceToTarget/2);
	    }
        } else {
